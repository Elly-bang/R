...
name =unlist(str_extract_all(EMP,'[가-힣]'[0-9]{3}'))
...
print_pay <- function(EMP) {
  
  avg <- 0
  library(stringr)
  for(i in 1:length(EMP)) {
    tmp <- str_extract_all(EMP[i], "[0-9]{3,}")
    tmp <- unlist(tmp)
    tmp <- as.numeric(tmp) 
