...
# 1) KoNLP 설치 
install.packages("KoNLP") #한글 자연어 관련 Ko
...
install.packages("https://cran.rstudio.com/bin/windows/contrib/3.4/KoNLP_0.80.1.zip",
                 repos = NULL) #R3.6.2파일에 저장하겠다.

# 2) Sejong 설치 : KoNLP와 의존성 있는 현재 버전의 Sejong 설치 
install.packages("Sejong") #세종 사전 

# 3) wordcloud 설치    
install.packages("wordcloud") #키워드 분석 결과를 시각화 , 단어 시각화

# 4) tm 설치 
install.packages("tm") #텍스터 마이닝 : 전처리, 행렬 DTM(문서 단어 행렬), TDM


# 5) 패키지 로딩
library(rJava)
library(KoNLP) #sejong은 koNLP의존성으로 자동 설치됨
#Checking user defined dictionary!

# ‘hash’이라고 불리는 패키지가 없습니다-> install.packages("hash")->library(KoNLP)재설치
...
## 4. 단어추출 사용자 함수 정의
# (1) 사용자 정의 함수 실행 순서 : 문장 -> 문자형 ->명사 추출 (Noun)-> 공백합침(collapse)
exNouns <- function(x) { 
...
facebook_nouns <- sapply(facebook_data, exNouns)  #facebook_data 76개의 문장 
facebook_nouns
# (3) 단어 추출 결과
str(facebook_nouns) 
facebook_nouns[1] 
...
myCorpusPrepro_term <- TermDocumentMatrix(myCorpusPrepro, 
                                          control=list(wordLengths=c(4,16))) #2절~8절 한글하나는 2바이트를 차지한다. 
                                                      #단어 행, 문서 열
...
str(myTerm_df)
myTerm_df[c(1:10),1] #1 1 1 1 2 1 1 1 3 1 : 특정단어에 대한 출현 빈도수 


## 7. 단어 빈도수 구하기
# (1) 단어 빈도수 내림차순 정렬
wordResult<- sort(rowSums(myTerm_df), decreasing=TRUE) 
wordResult[1:20] #1~10위까지 내림차순
