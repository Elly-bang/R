...
head(data[sort(data$cost),], 30) #index정렬 :sort

head(data[order(data$cost),], 30) #내용정렬 :order 기존.

library(dplyr)
arrange(data, cost) #오름차순 : 새로운 행이 만들어진다. 

#order vs arrange
#order : 기존 행 번호 유지 ,
#arrange : 새로운 행을 만들어 특정컬럼 기준으로 

#최빈수(mode)
mode() #data type제공 : "numeric"
...
#최빈수=중위수=평균 : 좌우대칭 
#최빈수 < 중위수:평균 :단봉 - 오른쪽기울어짐 (왜곡)
#중위수:평균 < 최빈수  : 단봉 - 왼쪽으로 기울어짐 (왜곡)
...
#정규성 검정 (모수 검정과 비모수 검정 chap10_43page)
shapiro.test(cost)
